      ...
      <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {cards.map((card, i) => (
            ...
            >
              <Card className={`border ${border} ${cardBg} hover:shadow-md transition-shadow`} theme={theme}>
        ...
        <Card className={`border ${border} ${cardBg} p-4 md:p-6 overflow-hidden`} theme={theme}>
          <h2 className="text-lg font-bold mb-6 flex items-center gap-2">
            <BarChart3 size={20} className="text-blue-500" />
            Performance Overview
          </h2>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%" className={`${text} ${cardBg}`}>
              <LineChart data={chartData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                />
                <Tooltip 
                    contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)', backgroundColor: `${bg}`}}
                />
                <Line
                  type="monotone"
            ...
            {recentBooks.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {recentBooks.map((book) => (
                  <Card key={book.id} className={`border ${border} ${cardBg} p-4 hover:border-blue-500/50 transition-colors`} theme={theme}>
